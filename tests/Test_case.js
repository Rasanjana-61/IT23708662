...
test("Pos_Fun_0002", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "assignment eka ivara karalaa mama nidhaagannavaa.",
  );
...
test("Pos_Fun_0003", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "lecture eka patan gannee kiiyatadha?",
  );
...
test("Pos_Fun_0005", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "sir aave naeththam api canteen ekata yamu.",
  );
...
test("Pos_Fun_0009", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "oyaalaa okkoma library enavadha?",
  );
...
test("Pos_Fun_0010", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "karuNaakaralaa mata deadline eka extend karanna.",
  );
...
test("Pos_Fun_0011", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "kohomadha machan, vaeda ivaradha?",
  );
...
test("Pos_Fun_0013", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "class eka thiyennee 8.30 AM ta.",
  );
...
test("Pos_Fun_0014", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "project presentation eka may 20.",
  );
...
test("Pos_Fun_0015", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "mata JAVA code eka email karanna puluvandha?",
  );
...
test("Pos_Fun_0017", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "anee mandhaa eyaa hinaa velaa  kathaa kalaa.",
  );
...
test("Pos_Fun_0018", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "mata OTP eka sms ekak vidhihata aavaa.",
  );
...
test("Pos_Fun_0019", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "iiyee raee godak vaehapu nisaa paraval okkoma vathuren vaehilaa thibunaa.",
  );
  const expected =
    "ඊයේ රෑ ගොඩක් වැහපු නිසා පරවල් ඔක්කොම වතුරෙන් වැහිලා තිබුනා.";
...
test("Pos_Fun_0020", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "adha havasa 5 ta meeting ekak thiyenavaa. oyaata puluvannam enna. naeththam record eka ahanna. api project eka gaena katha karanavaa.",
  );
  const expected =
    "අද හවස 5 ට meeting එකක් තියෙනවා. ඔයාට පුලුවන්නම් එන්න. නැත්තම් record එක අහන්න. අපි project එක ගැන කත කරනවා.";
...
test("Pos_Fun_0021", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "api iita passe party ekak dhamu!",
  );
...
test("Pos_Fun_0022", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "oyaa inne canteen ekeadha library ekeadha?",
  );
...
test("Pos_Fun_0024", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "oyaage laptop ekee thiyena final project file eka mata ikmanata email karanna puluvandha?",
  );
  const expected =
    "ඔයාගෙ laptop එකේ තියෙන final project file එක මට ඉක්මනට email කරන්න පුලුවන්ද?";
...
test("Neg_Fun_0001", async ({ page }) => {
  const actual = await getSinhalaTranslation(page, "mama adha cmpus yanavaa.");
...
test("Neg_Fun_0002", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "lectureekapatan gannekiiyatadha?",
  );
  const expected = "lecture එක පටන් ගන්නේ කීයටද?";
...
test("Neg_Fun_0004", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "assignment eka hariiii amarui",
  );
  const expected = " assignment එක හරි අමාරුයි";
...
test("Neg_Fun_0006", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "adha lecture eka hari amarui nisaa mata samahara dewal therum ganna amarui kiyala hithenavaa",
  );
  const expected =
    "අද lecture එක හරි අමරුඉ නිසා මට සමහර දෙවල් තෙරුම් ගන්න අමරුයි කියල හිතෙනවා";
...
test("Neg_Fun_0007", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "mata exam eka hariyata karanna baaa",
  );
...
test("Neg_Fun_0008", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "mata OTP eka SMS ekak vidihata avvaaa.",
  );
  const expected = "මට OTP එක sms එකක් විදිහට ආවා.";
...
test("Neg_Fun_0009", async ({ page }) => {
  const actual = await getSinhalaTranslation(
    page,
    "mama campus yanavaa lecture eka thiyenne",
  );
  const expected = "මම  කැම්පස් යනවා.    lecture එක පටන් ගන්නේ කීයටද?";
